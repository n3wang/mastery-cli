...
describe('Quizzer Hash-Based Selection Tests', () => {
	...
	beforeEach(function () {
		...
		testTerms = [
			...
			{
				term: 'Binary Tree',
				description: 'Tree with max 2 children per node',
	...
	afterEach(function () {
		// Clean up test files
		if (
			quizzer.termCompletionTracker &&
			fs.existsSync(quizzer.termCompletionTracker.filepath)
		) {
	...
	describe('Completion Count Tracking', () => {
		...
		it('should persist completion counts across sessions', async () => {
			...
			const newQuizzer = new Quizzer(
				[],
				[],
				mockMasterDeck,
				mockMasteryManager
			);
			newQuizzer.termCompletionTracker = new HashStorage(
				quizzer.termCompletionTracker.filename
			);
	...
	describe('Smart Selection Algorithm - Prioritizes Least Practiced', () => {
		...
		it('should prioritize least practiced terms', () => {
			const selectedTerms = quizzer.selectLeastPracticedTerms(
				testTerms,
				3
			);
			...
			const selectedHashes = selectedTerms.map(term =>
				quizzer.generateTermHash(term)
			);
			const selectedCounts = selectedHashes.map(hash =>
				quizzer.getTermCompletionCount(hash)
			);
		...
		it('should include unpracticed terms first', () => {
			...
			const unpracticedCount = selectedCounts.filter(
				count => count === 0
			).length;
			assert(
				unpracticedCount >= 1,
				'Should prioritize unpracticed terms'
			);
		});
		...
		it('should demonstrate selection strategy with specific scenario', () => {
			...
			const results = selectedTerms.map(term => ({
				term: term.term,
				count: quizzer.getTermCompletionCount(
					quizzer.generateTermHash(term)
				)
			}));
			...
			assert(
				hasUnpracticed || hasLeastPracticed,
	...
	describe('Real-world Usage Scenarios', () => {
		...
		it('should demonstrate adaptive learning behavior', async () => {
			...
			const newSelection = quizzer.selectLeastPracticedTerms(
				testTerms,
				2
			);
			...
			const unpracticedInNewSelection = newSelectionCounts.filter(
				count => count === 0
			).length;
			assert(
				unpracticedInNewSelection > 0,

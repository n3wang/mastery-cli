...
| Settings files | 6, across 3 independent loaders (§5) |
| Command registries | 3, mutually inconsistent — 2 documented-but-dead, 13 undocumented (§6) |
...
| From | To |
| --- | --- |
| `src/extensions/dsa-cli/tests/` | `src/extensions/dsa-cli/problem-sets/` |
| `src/extensions/dsa-cli/dsa_tests/` | `src/extensions/dsa-cli/__tests__/` (or move into top-level `tests/`) |
...
`snake_case` for JSON keys (already the majority there, and §5.4 makes it the settings rule).
...
Command names themselves are covered in §6.
...
were the product default. Whatever happens to the `schedule-assistant` feature (see §10),
this file needs a neutral replacement.

### 5.3 Target design

One user-scoped config file, one schema, one loader:

```
<OS config dir>/mastery-cli/config.json      # the ONLY live settings file
  {
    "version": 1,
    ...
    "extensions": {
      "dsa-cli": { ... },                    # namespaced per extension
      "schedule-assistant": { ... }
    }
  }
...
4. **Extensions namespace into the one file.** `ExtensionModel`'s `settingsPath` option is
   replaced by an `extensionSettings(name)` accessor backed by `config.extensions[name]`.
   `src/extensions/dsa-cli/settings-manager.js` is deleted; the DSA trainer reads
   `config.editor`.
5. **Config lives outside the package.** Follow the XDG / `%APPDATA%` convention (`env-paths`
   or a short equivalent) so `npm install -g` upgrades never clobber user settings, and the
   repo can never accumulate another committed live config.
...
## 6. Command & Help Standardization

### 6.1 Three registries that disagree
...
| `--debug, -d` | Declared and documented, but nothing branches on it (see §7.2 on logging). Either wire it to the logger or remove it. |
...
### 6.4 Target: a single command registry
...
| --- | --- | --- | --- |
| `co` / `coa` | `commit` | `co`, `coa` | fixes the §6.2 dead command |
...
**Practice:** `dsa` and `mdsa` → `dsa` and `dsa --markdown`; `cloze` and `jupyter` stay.

**Introspection:** `code` → `where` (it prints the install path; `code` reads as "open in
VS Code"); `setting` → `config` (fixes the singular/plural inconsistency and aligns with
§5.3's `config.json`); `skill` and `entries` → `report --skills` and `report --entries`.
...
score-bumping backdoor), `services` (already commented out of help), `demo-info` (fold into
`extensions`).
...
### 7.3 Confirmed bugs found while surveying
...
1. **`src/settings.js:41` and `src/SettingsManager.js:29`** — the "neither settings.json
   ...
   message. Same copy-pasted block in both files — see §5.2(d).
...
3. **`index.js:88`** — `input.includes(options[0]) && cli_meow.showHelp(0)` makes the
   first help-declared command (`co`) print help and exit instead of running. See §6.2.
4. **`package.json` `files` field** — see §2.4.

### 7.4 Tooling to add

- **ESLint** (`eslint:recommended` + `plugin:node/recommended`), wired into CI. Will
  surface the §7.3 class of bug automatically.
...
## 8. Documentation & Community Files
...
Each phase is a reviewable PR against an `oss-prep` branch. Phases 0–6 are the publish gate.

**Phase 0 — Decisions (blocking, no code)**
- [ ] Choose history strategy (clean history recommended) -> clean history
- [ ] Confirm canonical bin name (`mastery` + `mcli`) -> 'mcli' is the cannonical name
- [ ] Confirm the public repo name/org -> https://github.com/n3wang/mastery-cli
...
- [ ] Consolidate `.gitignore` (currently 3 generations of rules, including dead
      `utils/dsa-cli/…` paths and a Windows-backslash entry that git ignores)
- [ ] Verify `mastery report`, `mastery quiz`, `mastery dsa` still run

**Phase 2 — Content split**
- [ ] Author 3–4 original sample decks under `content/terms_modules/`
- [ ] Remove the existing `terms_modules` decks
- [ ] Move runtime state to the OS user-data dir; update `src/userDataPaths.js`
- [ ] Update the ~15 `getDirAbsoluteUri('user_data/…')` call sites
- [ ] First-run bootstrap: seed the user dir from `content/` on cold start

**Phase 3 — Settings consolidation (§5)**
- [ ] Delete `src/user_data/` (files 3 and 4) and drop the legacy-root fallback
...
- [ ] Fold `temp_settings.json` into `config.extensions['dsa-cli']`; delete
      `src/extensions/dsa-cli/settings-manager.js`
- [ ] Replace `schedule-settings.json` with `content/schedule.example.json`
- [ ] Move config to the OS config dir; add `version` + `migrate()` + load-time validation
- [ ] Regression-test: mask toggle, then session start, must see the same config object

**Phase 4 — Command & help unification (§6)**
- [ ] Build `src/commands/registry.js`; generate dispatch **and** help from it
- [ ] Fix the `co` dead-command bug in `index.js:88`
- [ ] Register the 13 undocumented commands or delete them (§6.5)
...
**Phase 5 — Tests green**
- [ ] Pick one runner (**Mocha** — 10 of 11 files already use it, and it is the declared devDependency) -> Choose mocha
- [ ] Convert `test_deletion_queue_storage.test.js` from Jest to Mocha
- [ ] Fix `makefile`'s `jest tests` target
- [ ] Repoint `test_md_problems_parser.test.js` at the extension parser
...
- [ ] Verify with `npm pack --dry-run` that the tarball contains a working CLI
- [ ] Add `LICENSE`
...
**Phase 10 — Post-publish (not blocking)**
- [ ] Split `utils.js` and `Quizzer.js` (§7.1)
- [ ] Structured logging wired to `--debug` (§7.2)
...
6. **Config location** — moving to the OS config dir (§5.3 rule 5) is correct for a
...
7. **Breaking the `*ses` commands** — the alias plan (§6.5) keeps every short form working

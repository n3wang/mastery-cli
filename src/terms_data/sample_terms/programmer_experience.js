...
const aws_services = [
	...
	},
	{
		...
		description:
			"While containerization technologies such as Docker are commonplace in cloud-native applications, we can take another step further by dividing our applications into discrete functions.\n\n\
	...
	},
	{
		...
		description:
			'Enterprise application integration (EAI) refers to the process of connecting different enterprise applications to allow data and functionality to be shared between them. Here are the pros and cons of EAI:\n\
	...
	},
	{
		...
		prompt: 'Suppose you have a program with 20 nodes and 30 edges in its control flow graph. What is the cyclomatic complexity of the program?',
		example:
			'Using the formula for cyclomatic complexity, we have:  cyclomatic complexity = number of nodes - number of edges + 2 = 20 - 30 + 2 = -8 + 2 = -6',
		description:
			'Cyclomatic complexity is calculated based on the control flow graph of the code, where each node in the graph represents a single point of execution and each edge represents a possible flow of control. The cyclomatic complexity of a program is defined as the number of nodes minus the number of edges plus two. This formula provides a measure of the minimum number of independent paths that must be tested to ensure that all paths through the code have been tested.'
	},
	{
		term: 'Questions-ch2',
        ...
        ',
		example:
			'In event-driven architecture, event producers and event consumers exchange messages through a message broker. A message broker acts as an intermediary between the event producer and the event consumer, allowing them to exchange messages asynchronously and decoupling their interactions. The message broker receives messages from event producers, stores them in a queue, and then delivers them to the event consumers. This helps to ensure reliability, as messages are stored and delivered even if the event consumer is temporarily unavailable.\n\
	...
	},
	{
		...
		description:
			'Event-driven architecture (EDA) is a software design pattern in which the flow of the program is determined by events or messages. In event-driven systems, individual components, or event emitters generate events or messages that are then processed by other components, or event handlers.\n\
	...
	},
	{
		...
		example:
			'With PostgreSQL, it supports sharding, which means that now you can run horizontal distributed queries for aggregations, as well as vertical distributed queries for Filtering ORS and AND\
	...
	},
	{
		...
		description:
			'Based on XML, it enables the near-real-time exchange of structured data between two or more network entities.\n\
	...
	},
	{
		...
		description:
			'You have: 1) Aurora RDS, 2) Aurora RDS Serverless, 3) Postgres, 4) ElasticSearch, 5) Elasticache, 6) DynamoDB, 7) MongoDB'
	},
	{
		...
		description:
			'Stack Actions Creaate Change set for current Stack -> Upload File, calculate and then makes the changes\n\
	...
	{
		term: 'installing-latest-pip',
		prompt: 'How do you install the latest pyspark that starts with 2.4?',
		description: '*',
...
const coderTerms = [
	...
	,
	{
		term: 'Create Dynamic Frame from Catalog',
		prompt: 'Create a dynamic frame from a catalog in Glue using table database being p_db, and table name is customers and then print it',
		example:
			"dyf = glueContext.create_dynamic_frame.from_catalog(database='p_db', table_name='customers')\n\
        dyf.printSchema()",
		description:
	...
	},
	{
		...
		example:
			"dyf = dyf.rename_field('first_name', 'first')\ndyf = dyf.rename_field('last_name', 'last')",
	...
	},
	{
		...
		example:
			"mapping = [('fullname', 'string', 'name', 'string')]\n\
	...
	},
	{
		...
		example:
			'glueContext.write_dynamic_frame.from_options(frame = dyf, connection_type = "s3", connection_options = {"path": "s3://pyspark-dyf/"}, format = "parquet", transformation_ctx = "datasink4")',
		description:
	...
	// Advantages of REST, Response Codes, and distinction of HTTPS methods
	{
		...
		example:
			'Allows more flexibility for the front end, and can have now multiple front ends, and limitation of information we dont need as a response. No more thinking about endpoints, but now as a Schema\n\
	...
	},
	{
		...
		example:
			'Security possible, since you dont want to return sensitive information. Also caching becomes more tricky.\n\
	...
	},
	{
		...
		example:
			'XML is more verbose than REST, uses more bandwidth, XML can have actions or linked entities that can be nasty'
	},
	{
		term: 'grcp-characteristics',
		prompt: 'Whats are the characteristics of GRPC?',
		example:
			'Implements remote procedure calls, allows your code to call servers via compiled function calls, can compile for different languages but work similarly.\n\
	...
	// Own Development Rules
	{
		...
		example:
			'In Notion page, as automatically can be linked on github, also better for search'
	},
	{
		term: 'CASCADE',
		prompt: 'What does cascade deletion option do? When to use it?',
		example:
			'When a row in the parent table is deleted, all associated rows in the child table are also deleted. It is useful when you want to ensure that child records are always deleted along with the parent record to maintain data integrity.'
	},
	{
		term: 'SET NULL',
		prompt: 'What does set null deletion option do? When to use it?',
		example:
			'When a row in the parent table is deleted, the foreign key columns in the child table are set to NULL. It is useful when you want to remove the association between the parent and child records without deleting the child records themselves.'
	},
	{
		term: 'SET DEFAULT',
		prompt: 'What does set default deletion option do? When to use it?',
		example:
			'When a row in the parent table is deleted, the foreign key columns in the child table are set to their default values defined in the table schema. It is useful when you want to provide default values for the foreign key columns instead of leaving them NULL.'
	},
	{
		term: 'RESTRICT',
		prompt: 'What does restrict deletion option do? When to use it?',
		example:
			'Prevents deletion of a row in the parent table if there are associated rows in the child table. It enforces referential integrity and avoids orphaned child records. It is useful when you want to ensure that the parent record cannot be deleted as long as there are associated child records.'
	},
	{
		term: 'NO ACTION',
		prompt: 'What does no action deletion option do? When to use it?',
		example:
			"Similar to RESTRICT, it prevents deletion of a row in the parent table if there are associated rows in the child table. However, it may depend on the database engine's default behavior for handling such cases. It is useful when you want to rely on the default behavior of the database engine for handling referential integrity."
	},
	{
...
const docker = [
	...
	},
	{
		...
		term: 'docker run',
		example:
			'docker run -e AWS_ACCESS_KEY_ID=hello --name job-listener -d imagename'
	},
	{
...
const unit_testing = [
	...
	{
		term: '(a) Shared, (b) private, and (c) out-of-process dependencies (d) volatile (e) Collaborator, (f) Cyclomatic Complexity',
	...
	},
	{
		...
		example:
			'(2) Means that probably is an integration test and is time to split it into multiple tests. (3) That would be an antipattern. (4) Act should be one line, Assertion if too large means lack of abstraction and is okay for arrange to be large.'
	},
	{
		...
		example:
			"(1) Sum_of_two_numbers, delivery_with_pastdate_is_invalid // More specific why it's invalid, it's okay to be verbose."
	},
	{
		term: 'Good Pillars Tests',
		prompt: 'What mental guidelines to have to create good code?',
		description:
		...
		example:
			'(1) To maximize the metric of protection against regressions, the test needs to aim at exercising as much code as possible.\
        (2) Aim to keep distance between the actual (current implementation of the class.) \
        '
	},
	{
		term: 'mock stub',
		prompt: 'Describe difference Mock and Stub, when would you use each?',
		description:
	...
	{
		term: 'CQRS Principle',
		prompt: 'Whats the principle useful for?',
		attachment: './img/2023-02-02-10-30-14.png',
		description:
	...
	{
		term: 'Encapsulating APIs',
		prompt: 'What mental guidelines to follow when encapsulating API?',
		attachment: './img/2023-02-02-10-51-02.png',
		description:
			'\
	...
	{
		term: 'Hexagonal Architecture Enphasis',
		prompt: 'How would you refactor or use this Pattern on your projects?',
		attachment: './img/2023-02-02-12-11-43.png',
		description:
	...
	{
		term: '(a) Mathematical functions',
		prompt: 'Why identifying this is important?',
		example:
			'The goal is to cover the functional core exten- sively with output-based tests and leave the mutable shell to a much smaller number of integration tests',
		description:
			'\
        \
	...
	{
		term: 'Humble Object Pattern',
		prompt: 'When and why would be convenient to use it? What preconditions to test and what not to test?',
		description:
			'For example you have a code with logs of logic and time to setup, or also an ',
		example:
			'You can break it down into Factory pattern to build the objects, (with those preconditions)'
	},
	{
		term: 'end to end vs integration test',
		prompt: 'What is the difference between integration and end to end test?'
	},
	{
		term: 'integration testing',
		prompt: 'Think of how would you design a passable Integration Testing for your current project?',
		description:
	...
	{
		term: 'support, diagnostic',
		prompt: 'Whats the difference?',
		description:
	...
	{
		term: 'Managaged, Unmanaged',
		prompt: 'Whats the difference? For which should you create a test?',
		description:
	...
	{
		term: 'YAGNI',
		prompt: 'Why Yagni is a good principle? Not good? Why would you use an interface with a single implementation?',
		description:
	...
	{
		term: 'Spy',
		promp: 'Whats the difference between Mocks and Spies?',
		description:
...
const js_advanced = [
	...
	},
	{
		...
		example:
			'Memory leaks in JavaScript occur when there is a build-up of unreferenced objects in memory. This can happen when objects are created but not properly released from memory, or when circular references between objects prevent the garbage collector from removing them.'
	},
	{
	...
	},
	{
		...
		example:
			'The main difference between a promise and a callback is that a callback is a function that is passed as an argument to another function, while a promise is an object that represents the eventual completion or failure of an asynchronous operation. Promises are more flexible than callbacks because they can be chained, which allows them to be used to handle asynchronous operations that depend on the result of a previous operation. Promises also provide a way to handle errors that occur in asynchronous operations, which is not possible with callbacks.\n\
	...
	},
	{
		...
		example:
			'It will throw an error because doodle is not accessible outside its scope'
	},
	{
		term: 'IIFE',
	...
	},
	{
		...
		example:
			"Context is the object that the function is a property of. Scope is the variable environment of the execution context, which consists of any local variables that were in-scope at the time the execution context was created.\n\n\
	...
	},
	{
		...
		example:
			'const clone = [...arr]; | const clone = arr.slice(); | const clone = arr.concat();'
	},
	{
		...
		example:
			'const clone = { ...obj }; | const clone = Object.assign({}, obj);'
	},
	// TELL to chatgpt: Convert the following into a one line markdown format with :m at the start and surroudn it with "" e.g. function marry(person1, person2){ console.log(arguments); console.log(Array.from(arguments)); return ${person1} is now married to ${person2}; }\n\nmarry('Tim', 'Tina'); to ":m\n\ ```js\n\         function marry(person1, person2) {\n\  \tconsole.log(arguments);\n\  \tconsole.log(Array.from(arguments));\n\  \treturn `${person1} is now married to ${person2}`;\n\  \  marry('Tim', 'Tina');\}\n\```""
	{
        ...
        \n\
          ...
          ```",
		example:
			'Arguments(2)[\'Tim\', \'Tina\', callee: ƒ, Symbol(Symbol.iterator): ƒ]\n\
        (2) ["Tim", "Tina"]\n\
...
const best_practices = [
	...
	},
	{
		term: 'js | naming vars',
		prompt:
			'How would you name a variable for calculation result of the price?\n\
        use nouns for variable names: ' +
			`names should be nouns that describe the value being stored. For example, "customerName" or "orderTotal".`,
		example: 'priceResult',
...
const git = [
	...
	},
	{
		...
		example:
			'Stagging is the area where you add the files you want to commit, Diff is the area where you can see the changes you made to the files',
		description:

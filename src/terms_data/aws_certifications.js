...
const aws_certification_cloud_practitioner = [
	...
	},
	{
		...
		example:
			'AWS Redshift, Redshift shines in its ability to handle huge volumes of data — capable of processing structured and unstructured data in the range of exabytes (1018 bytes). However, the service can also be used for large-scale data migrations. \n\
...
const aws_certification_associate_developer = [
	...
	},
	{
		...
		description:
			':m SSE-S3 server-side encryption protects data at rest. Amazon S3 encrypts each object with a unique key. As an additional safeguard, it encrypts the key itself with a key that it rotates regularly. Amazon S3 server-side encryption uses one of the strongest block ciphers available to encrypt your data, 256-bit Advanced Encryption Standard (AES-256).  You can use the following bucket policy to deny permissions to upload an object unless the request includes the x-amz-server-side-encryption header to request server-side encryption using SSE-S3:\n\n\
        ```json\n\
	...
	},
	{
		...
		description:
			"EC2 burst balance is a feature of Amazon Elastic Compute Cloud (EC2) instances that allows instances to temporarily use additional CPU credits beyond their baseline performance level.\n\
	...
	// TODO Create the terms here:
	{
		...
		example:
			':m Use `AWS CloudTrail` to get a record of actions taken by a user  \n\
		...
		description:
			':m INCORRECT: Use SSM Parameter Store List feature to get a record of actions taken by a user - This option has been added as a distractor.  \n\n\
	...
	},
	{
		...
		example:
			':m You need to `activate IAM user access` to the Billing and Cost Management console for all the users who need access - \n\n\
	...
	},
	{
		...
		example:
			':m ### Access Advisor feature on IAM console\n\
	...
	},
	{
		...
		example:
			':m ### Enable CodeBuild timeouts     A build represents a set of actions performed by AWS CodeBuild to create output artifacts (for example, a JAR file) based on a set of input artifacts (for example, a collection of Java class files).  \n\
	...
	},
	{
		...
		example:
			':m AWS CloudFormation template is a JSON- or YAML-formatted text file that describes your AWS infrastructure. \n\
	...
	},
	{
		...
		example:
			'U:m se the `--region parameter`: If the region parameter is not set, then the CLI command is executed against the default AWS region.  You can also review all general options for AWS CLI: ',
	...
	},
	{
        ...
        ',
		example:
			' Auto Saling groups that span across multiple Regions need to be enabled for all the Regions specified \
	...
	},
	{
		...
		example:
			".ebextensions/<mysettings>.config : You can add AWS Elastic Beanstalk configuration files (.ebextensions) to your web application's source code to\
	...
	},
	{
		...
		example:
			':m Correct option:  `"Cognito User Pools"`  After successful authentication, Amazon Cognito returns user pool tokens to your app. \
	...
	},
	{
		...
		description:
			':m `User Pools` are used to `manage user authentication and authorization`, and they provide features such as user `sign-up`, \
		...
		example:
			':m You can use `User pool` for developing an app where `users can sign up`, while `Identity Pool` is for your applications to be able to \
	...
	},
	{
		...
		example:
			":m `AWS Cloud Development Kit (CDK)` - The AWS Cloud Development Kit (AWS CDK) is an open-source software development framework to define your cloud application resources using familiar programming languages.  \n\
	...
	},
	{
		...
		example:
			':m Deploy the function with its `memory allocation` set `to the maximum amount` \n\
	...
	},
	{
		...
		example:
			":m Enable API caching in API Gateway  API Gateway provides a few strategies for optimizing your API to improve responsiveness, like `response caching and payload compression`.\
	...
	},
	{
		...
		example:
			":m ElastiCache defined in .ebextensions/ - Any resources created as part of your .ebextensions is part of your Elastic Beanstalk template and `will get deleted if the environment is terminated`.  \n\
	...
	},
	{
		...
		example:
			':m A `load balancer` accepts incoming traffic from clients and routes requests to its registered targets (such as EC2 instances) in one or more Availability Zones.  \n\
		...
		description:
			'INCORRECT: 25 - If cross-zone load balancing is disabled, each of the two targets in AZ1 will receive 25% of the traffic. Because the load balancer is only able to send to the targets registered in AZ1 (AZ2 instances are not accessible for load balancer on AZ1)  20 - Invalid option, given only as a distractor.  15 - Invalid option, given only as a distractor.'
	},
	{
		...
		example:
			'`Amazon EFS volumes` - EFS volumes provide a `simple, scalable, and persistent file storage` for use with your Amazon ECS tasks. \
	...
	},
	{
		...
		example:
			':m Elastic Beanstalk will replace them with `instances running the application version` from the most recent successful deployment      \n\
	...
	},
	{
		...
		example:
			':m `KMS stores the CMK`, and `receives data` from the clients, which it encrypts and sends back   \n\
	...
	},
	{
        ...
        - X-Ray',
		example:
			':m `X-Ray`  AWS X-Ray helps developers analyze and debug production, distributed applications, such as those built using a microservices architecture. With X-Ray, \
	...
	},
	{
		...
		example:
			":m Create the app from a template provided by AWS CDK \n\
		...
		description:
			`The standard AWS CDK development workflow is similar to the workflow you're already familiar as a developer. \n\
	...
	},
	{
        ...
        AWS Marketplace',
		example:
			":m AWS Serverless Application Repository (SAR) - The AWS Serverless Application Repository is a managed repository for serverless applications. It enables teams, organizations, and individual developers to store and share reusable applications, \
	...
	},
	{
		...
		description:
			':m `AWS CloudFormation StackSets` extends the capability of stacks by enabling you to create, update, or delete stacks across multiple accounts and AWS Regions with a single operation. Using an administrator account, you define and manage an AWS CloudFormation template, \
	...
	},
	{
		...
		example:
			':m Create a `development stage` on the API Gateway API and then have the `developers point the endpoints to the development stage`',
		attachment: './img/2023-04-24-09-26-10.png',
		description:
			":m A stage is a `logical reference` to a lifecycle state of your API (for example, 'dev', 'prod', 'beta', 'v2'). \
	...
	},
	{
		...
		example:
			'Resources: Templates include several major sections. The Resources section is the only required section.',
	...
	},
	{
		...
		example:
			'Correct option:  AWS Budgets lets customers set custom budgets and receive alerts if their costs or usage exceed (or are forecasted to exceed) their budgeted amount.',
		description:
	...
	},
	{
		...
		example:
			':m `Trust policy` - Trust policies define which principal entities (accounts, users, roles, and federated users) can assume the role. An IAM role is both an identity and a resource that supports resource-based policies. For this reason, you must attach both a trust policy and an identity-based policy to an IAM role. The IAM service supports only one type of resource-based policy called a role trust policy, which is attached to an IAM role.'
	},
	{
	...
	},
	{
		...
		example:
			':m `AWS::Serverless::Function` - This resource creates a Lambda function, IAM execution role, and event source mappings that trigger the function.  \n\
	...
	},
	{
		...
		example:
			"DeleteQueue - Deletes the queue specified by the QueueUrl, regardless of the queue's contents. When you delete a queue, any messages in the queue are no longer available.  When you delete a queue, the deletion process takes up to 60 seconds. Requests you send involving that queue during the 60 seconds might succeed. For example, a SendMessage request might succeed, but after 60 seconds the queue and the message you sent no longer exist.  When you delete a queue, you must wait at least 60 seconds before creating a queue with the same name.",
		description:
	...
	},
	{
		...
		description:
			'Incorrect options:  Use AWS Billing Usage Plans - AWS Billing and Cost Management is the service that you use to pay your AWS bill, monitor your usage, and analyze and control your costs. There is no such thing as AWS Billing Usage Plans. You cannot use AWS Billing to set up public APIs for the application.  Use CloudFront Usage Plans - Amazon CloudFront is a fast content delivery network (CDN) service that securely delivers data, videos, applications, and APIs to customers globally with low latency, high transfer speeds, all within a developer-friendly environment. There is no such thing as CloudFront Usage Plans. You cannot use CloudFront to set up public APIs for the application.  Use AWS Lambda Custom Authorizers - Lambda is a separate service than Gateway API, therefore, it cannot be used to determine the API usage limits.'
	},
	{
	...
	},
	{
		...
		description:
			':m Pseudo parameters are parameters that are predefined by AWS CloudFormation. You do not declare them in your template. Use them the same way as you would a parameter, as the argument for the Ref function.  \n\

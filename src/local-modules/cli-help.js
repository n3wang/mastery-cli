...
`;
    ...
    if (commandEntries.length > 0) {
        const maxCommandLength = Math.max(...commandEntries.map(([cmd]) => cmd.length));
    ...
    if (flagEntries.length > 0) {
        const maxFlagLength = Math.max(...flagEntries.map(([flag]) => {
        ...
        for (const [flag, flagInfo] of flagEntries) {
            ...
            const defaultValue = flagInfo.default !== undefined ? ` (default: ${flagInfo.default})` : '';

class LargestRectangleArea {
	 ...
	 * @return {number}
	 */
	solve2(heights, left = 0, right = heights.length - 1) {
	 ...
	 * @return {number}
	 */
	solve3(heights, left = 0, right = heights.length - 1) {
		const divideAndConquer = (heights, left, right, min = left) => {
			for (let i = left; i <= right; i++) {
				/* Time O(N) */
				const isMinGreater = heights[i] < heights[min];
	...
	solve4(heights) {
		const fillStack = (heights, stack = [], maxArea = 0) => {
			for (let index = 0; index < heights.length; index++) {
				...
				const isCurrHeightLess = (
					[prevIndex, prevHeight],
					currHeight
		...
		const { stack, maxArea } =
			fillStack(heights); /* Time O(N) | Space O(N) */

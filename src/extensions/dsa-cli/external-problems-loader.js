...
const { parseMarkdownProblemsFromFolder, convertToProblemsMetadata } = require('./md_dsa_parser.js');
...
class ExternalProblemsLoader {
    constructor() {
        this.tempSettingsPath = getDirAbsoluteUri('./user_files/temp_settings.json', './');
    ...
    loadTempSettings() {
        try {
            if (!fs.existsSync(this.tempSettingsPath)) {
                console.warn(`temp_settings.json not found at ${this.tempSettingsPath}`);
                return { external_problems_folders: [], settings: {} };
            }

            const settingsContent = fs.readFileSync(this.tempSettingsPath, 'utf-8');
            const settings = JSON.parse(settingsContent);
            
            return {
                external_problems_folders: settings.external_problems_folders || [],
    ...
    loadExternalProblems() {
        ...
        if (!external_problems_folders || external_problems_folders.length === 0) {
        ...
        for (const folderPath of external_problems_folders) {
            try {
                if (this.loadedFolders.has(folderPath)) {
                    if (settings.log_external_loading) {
                        console.log(`Folder ${folderPath} already loaded, skipping`);
                    }
                    continue;
                }

                if (!fs.existsSync(folderPath)) {
                    console.warn(`External problems folder does not exist: ${folderPath}`);
                    continue;
                }

                if (settings.log_external_loading) {
                    console.log(`Loading external problems from: ${folderPath}`);
                }
                ...
                if (settings.log_external_loading) {
                    console.log(`Loaded ${problemMetadata.length} problems from ${folderPath}`);
                }

            } catch (error) {
                console.error(`Error loading problems from ${folderPath}:`, error.message);
            }
        }
        ...
        if (settings.log_external_loading && allProblems.length > 0) {
            console.log(`Total external problems loaded: ${allProblems.length}`);
        }
    ...
    addExternalFolder(folderPath) {
        try {
            ...
            if (!config.external_problems_folders.includes(folderPath)) {
                ...
                const currentSettings = JSON.parse(fs.readFileSync(this.tempSettingsPath, 'utf-8'));
                currentSettings.external_problems_folders = config.external_problems_folders;
                currentSettings.last_updated = new Date().toISOString();
                
                fs.writeFileSync(this.tempSettingsPath, JSON.stringify(currentSettings, null, 4));

...
const commands = {
	help: function () {
		this.echo(
			`avaiable chars: ${charts_commands}, [[b;#fff;]gauge] and [[b;#fff;]scatter]`
		);
		this.echo(
			[
				'',
		...
		this.echo(
			'additional commands: [[b;#fff;]credits], [[b;#fff;]demo], [[b;#fff;]size] and [[b;#fff;]help] '
		);
	},
	gauge: function (...args) {
		console.log(args);
		...
		this.echo(
			ervy.gauge([data], { radius: options.r || 7, style, bgStyle })
		);
	},
	scatter: function (...args) {
		...
		var plot = ervy.scatter(data, {
			width: options.width || 15,
			legendGap: options['legend-gap'] || 18
		});
	...
	demo: function () {
		this.exec(
			charts
				.map(
					name => `${name} A:10:green B:20:yellow C:30:red d:10:blue`
				)
				.concat([
					'gauge Foo:.2:green -b red',
